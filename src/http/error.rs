...
pub enum Error {
    ...
    /// Return `500 Internal Server Error` on a `OpenAIError`.
    /// Via the generated `From<OpenAIError> for Error` impl, this allows the
    /// use of `?` in handler functions to automatically convert `OpenAIError` into a response.
    /// Like with `Error::Sqlx`, the actual error message is not returned to the client
    /// for security reasons.
    #[error("an error occurred with the OpenAI API")]
    OpenAI(#[from] OpenAIError),

    #[error("an error occurred with std::io")]
            ...
            Self::UnprocessableEntity { errors } => {
                ...
                return (
                    ...
                    Json(CommonResponse {
                        ...
                        message: "error in the request body".to_string(),
